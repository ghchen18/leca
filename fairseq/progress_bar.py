...
def build_progress_bar(args, iterator, epoch=None, prefix=None, default='tqdm', no_progress_bar='none'):
    ...
    if args.log_format == 'json':
        ...
        bar = simple_progress_bar(iterator, epoch, prefix, args.log_interval, expname=args.exp_name)
...
class simple_progress_bar(progress_bar):
    ...
    def __init__(self, iterable, epoch=None, prefix=None, log_interval=1000, expname=None):
        super().__init__(iterable, epoch, prefix)
        self.log_interval = log_interval
        self.stats = None
    ...
    def __iter__(self):
        ...
        for i, obj in enumerate(self.iterable):
            ...
            if self.stats is not None and i > 0 and \
                ...
                print('|{} | {}:  {:5d} / {:d} {}'.format(self.expname, self.prefix, i, size, postfix),
                      flush=True)
...
class tensorboard_log_wrapper(progress_bar):
    ...
    def _writer(self, key):
        ...
        if key not in self._writers:
            logtime = time.strftime("%m%d_%H%M", time.localtime())
            self._writers[key] = self.SummaryWriter(
                log_dir=os.path.join(self.tensorboard_logdir, key, logtime),
            )
