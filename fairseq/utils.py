...
def save_state_emnlp18(filename, args, model_state_dict, criterion, optimizer, lr_scheduler,
               num_updates, optim_history=None, extra_state=None):
    if optim_history is None:
        optim_history = []
    if extra_state is None:
    ...
    state_dict = {
        ...
        'optimizer_history': optim_history + [
            {
                'criterion_name': criterion.__class__.__name__,
                'optimizer_name_nmt': optimizer[0].__class__.__name__,
                'optimizer_name_adv': optimizer[1].__class__.__name__,
                'lr_scheduler_state': lr_scheduler.state_dict(),
                'num_updates': num_updates,
            }
        ],
        'last_optimizer_state': [convert_state_dict_type(optimizer[0].state_dict()), \
                                 convert_state_dict_type(optimizer[1].state_dict())],
        'extra_state': extra_state,
    }
    torch_persistent_save(state_dict, filename)
...
def _upgrade_state_dict(state):
    ...
    if 'lr_scheduler_state' not in state['optimizer_history'][-1]:
        state['optimizer_history'][-1]['lr_scheduler_state'] = {
            # 'best_loss': state['optimizer_history'][-1]['best_loss'],
            'best': state['optimizer_history'][-1]['best_loss'], ##underlying bug
            'best_bleu': state['optimizer_history'][-1]['best_bleu']
        }
        del state['optimizer_history'][-1]['best_loss']
        del state['optimizer_history'][-1]['best_bleu']
...
def load_ensemble_for_inference(filenames, task, model_arg_overrides=None):
    ...
    for state in states:
        ...
        model.upgrade_state_dict(state['model'])
        print('Num of updates: ', state['optimizer_history'][-1]['num_updates'])
        print('Num of parameters: ', sum(torch.numel(elem) for elem in model.parameters()))
...
def _get_full_incremental_state_key(module_instance, key):
    ...
    # initialize the _fairseq_instance_id, only run once for all running process
