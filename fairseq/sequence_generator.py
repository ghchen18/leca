...
class SequenceGenerator(object):
    def __init__(
        ...
        if sampling:
            ...
            self.search = search.LengthConstrainedBeamSearch(
                tgt_dict, min_len_a=1, min_len_b=0, max_len_a=1, max_len_b=10,
            )
        else:
    ...
    def generate(
        ...
        **kwargs
        ...
        # initialize buffers  fairseq/sequence_generator.py:158
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker fairseq/sequence_generator.py:305
            ...
            lprobs, avg_attn_scores = model.forward_decoder(tokens[:, :step + 1], encoder_outs, reorder_state=reorder_state)
            ...
            # reorder incremental state in decoder  tokens[11:20]
...
class EnsembleModel(torch.nn.Module):
    ...
    def forward_encoder(self, encoder_input):
        ...
        return [model.encoder(src_tokens, src_lengths, decoder=model.decoder) for model in self.models]

    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs, reorder_state=None):
        ...
        for model, encoder_out in zip(self.models, encoder_outs):
            probs, attn = self._decode_one(tokens, model, encoder_out, self.incremental_states, log_probs=True, reorder_state=reorder_state)
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs, reorder_state=None):
        if self.incremental_states is not None:
            decoder_out = list(model.decoder(tokens, encoder_out, incremental_state=self.incremental_states[model], reorder_state=reorder_state))
        else:
            decoder_out = list(model.decoder(tokens, encoder_out, reorder_state=reorder_state))
